...
import transformers
from trainer_gcq_qa import QuestionAnsweringTrainer
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, GCQArguments, TrainingArguments)
    )
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, gcq_args, training_args = parser.parse_json_file(
    ...
    gcq_args.hidden_size = config.hidden_size
